...
def verify_pmid(pmid):
    ...
    if not pmid or len(pmid)<5: return False
    try:
        r=requests.get(f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id={pmid}&retmode=json",timeout=10)
        data=r.json()
...
def clean_pmids(content):
    ...
    for pmid in pmids:
        if not verify_pmid(pmid):
            content=re.sub(rf'PMID[:\s]*?{pmid}','[PMID removed - unverified]',content)
    ...
    try:
        r=requests.post('https://api.x.ai/v1/chat/completions',
            ...
            json={'model':'grok-3','max_tokens':600,'temperature':0.2,
                  'messages':[{'role':'user','content':f'Research "{topic}": key stats, AAN/NICE/AANS guidelines, 3 real PubMed PMIDs, Pakistan context. 400 words max.'}]},
            timeout=60)
        b=r.json()['choices'][0]['message']['content']
        print(f"Grok: {len(b.split())}w"); return b
    except Exception as e: print(f"Grok error: {e}"); return ""

def claude_generate(topic,kw,brief):
    research=f"\n\nGROK RESEARCH:\n{brief}" if brief else ""
    system=f"You are author of The Neuro Council, writing as {AUTHOR}. Write authoritative, evidence-based, SEO-optimised neuroscience content. Always include author byline and CTA: {CTA}. Only cite PubMed references you are CERTAIN exist - never fabricate PMIDs. Educational disclaimer required."
        ...
        for attempt in range(2):
            try:
                ...
                if r.status_code==200:
                    ...
                    print(f"SUCCESS: {len(t.split())}w | {model}")
                    return clean_pmids(t)
...
def notify(subject,body):
    if GMAIL_USER and GMAIL_PASS:
        try:
            msg=MIMEText(body); msg['Subject'],msg['From'],msg['To']=subject,GMAIL_USER,GMAIL_USER
            with smtplib.SMTP_SSL('smtp.gmail.com',465) as s:
                s.login(GMAIL_USER,GMAIL_PASS); s.send_message(msg)
            print("Gmail sent")
        except Exception as e: print(f"Gmail error: {e}")
    if TG_TOKEN and TG_CHAT:
        try: requests.post(f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage",json={'chat_id':TG_CHAT,'text':body},timeout=10); print("Telegram sent")
        except: pass

def main():
    print("="*55); print("THE NEURO COUNCIL v2.5 FINAL")
    print(datetime.now().strftime('%A %d %B %Y %H:%M PKT'))
    print(f"Mode: {PUBLISH_MODE}"); print("="*55)
    td=get_topic(); topic,kw=td['t'],td['k']; print(f"Topic: {topic}")
    ...
    notify(f"Neuro Council: {topic}",f"Topic: {topic}\nWords: {wc:,}\nMode: {PUBLISH_MODE}\nURL: {url}")
    print("="*55); print(f"DONE - {wc:,}w | {url}"); print("="*55)
