#!/usr/bin/env python3
"""
THE NEURO COUNCIL - Production Engine v2.2
Dr. Wasif Rizwan Malik | PMDC 47983-P | drwasifmalik.com
FIXED: model=claude-sonnet-4-20250514, timeout=180s, 3x retry, 18 topics
"""
...
def grok_research(topic):
    if not GROK_KEY:
        print("No GROK_API_KEY in secrets - add it to GitHub repo secrets")
        return ""
    ...
    try:
        r = requests.post('https://api.x.ai/v1/chat/completions',
            ...
            json={'model':'grok-3','max_tokens':800,'temperature':0.2,
                  'messages':[{'role':'user','content':f'Research neuroscience article on "{topic}". Latest trials 2023-2026, statistics, AAN/NICE/AANS guidelines, 5 PubMed citations with PMID, Pakistan context. Max 500 words.'}]},
            timeout=60)
...
def claude_generate(topic, pillar, keywords, grok_brief):
    if not CLAUDE_KEY:
        raise ValueError("No Claude key in ANTHROPIC_API_KEY secret")
    print(f"CLAUDE generating: {topic}")
    print(f"Model: {MODEL} | Timeout: 180s")
...
SEO-optimised. Cite PubMed evidence where possible."""

    prompt = f"""Write a complete SEO blog article for: **{topic}**
Primary keyword: {kw}{research}

Structure (target 1500-1800 words total):
...
## What is {topic.split(':')[0].strip()}? (200 words)
## Causes and Risk Factors (150 words)
## Symptoms and Warning Signs (200 words - bold red flags)
## Diagnosis (150 words - what to expect at Faraz Hospital)
## Treatment Options (250 words - evidence-based, conservative to surgical)
## Recovery and Outcomes (150 words)
## Frequently Asked Questions (5 Q&As, 200 words)
## Conclusion + Consultation CTA (100 words)

References: 6-8 PubMed citations [Author et al. Journal Year. PMID: XXXXX]
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"SUCCESS: {len(t.split())} words generated")
                return t
            print(f"Claude error {r.status_code}: {r.text[:300]}")
        except requests.exceptions.ReadTimeout:
            print(f"Timeout attempt {attempt+1}. Waiting 15s...")
            time.sleep(15)
        except Exception as e:
            print(f"Error: {e}")
            break
    raise RuntimeError("Claude failed after 3 attempts. Check status.anthropic.com")
    ...
    try:
        r = requests.post('https://api.anthropic.com/v1/messages',
            ...
            json={'model':MODEL,'max_tokens':1500,
                  'messages':[{'role':'user','content':f'Social media suite for "{topic}".\nBased on: {content[:500]}...\nCreate: Facebook EN (180w), Facebook Urdu Nastaliq (180w), Instagram + 15 hashtags including #NeurosurgeryPakistan #DrWasifMalik, Twitter thread (5 tweets), WhatsApp EN+Urdu (80w each). All branded: {AUTHOR_BRAND}, WhatsApp +923458254232.'}]},
            timeout=120)
        if r.status_code == 200:
...
def main():
    print("="*60)
    print("THE NEURO COUNCIL v2.2 - Production Engine")
    print(f"{datetime.now().strftime('%A %d %B %Y %H:%M PKT')}")
    print(f"Type: {CONTENT_TYPE} | Mode: {PUBLISH_MODE}")
    print(f"Model: {MODEL} | Timeout: 180s | Retries: 3")
    print("="*60)
    ...
    print("\n"+"="*60)
    print(f"COMPLETE - {wc:,} words | {MODEL}")
    if result:
...
Dr. Wasif Rizwan Malik | PMDC 47983-P | drwasifmalik.com
KEY FIXES: timeout=180, model=claude-sonnet-4-5, 3x retry, 18-topic pool
"""
import os, requests, re, time
from datetime import datetime
...
TOPIC_POOL = [
    ...
    {"topic": "Parkinson disease: from diagnosis to deep brain stimulation", "pillar": "patient_education", "keywords": ["Parkinson disease Pakistan","DBS tremor"]},
    {"topic": "CVST: the stroke that strikes young women after childbirth", "pillar": "clinical_advances", "keywords": ["CVST","cerebral venous sinus thrombosis"]},
    {"topic": "Bell palsy: complete recovery is possible", "pillar": "patient_education", "keywords": ["Bell palsy","facial nerve","facial palsy"]},#!/usr/bin/env python3
"""
THE NEURO COUNCIL - Production Engine v2.0
Dr. Wasif Rizwan Malik | PMDC 47983-P | drwasifmalik.com
KEY FIXES: timeout=180, model=claude-sonnet-4-5, 3x retry, 18-topic pool
...
TOPIC_POOL = [
    ...
    {"topic": "Bell palsy: complete recovery is possible", "pillar": "patient_education", "keywords": ["Bell palsy","facial nerve","facial palsy"]},#!/usr/bin/env python3
"""
THE NEURO COUNCIL - Production Engine v2.1
Dr. Wasif Rizwan Malik | PMDC 47983-P | drwasifmalik.com
Model: claude-sonnet-4-5 | Timeout: 180s | 3x retry | 18 topics
"""
import os, requests, re, time
from datetime import datetime
...
def grok_research(topic):
    if not GROK_KEY:
        print("No Grok key in secrets - skipping research")
        return ""
    print(f"GROK: {topic}")
    try:
        r = requests.post('https://api.x.ai/v1/chat/completions',
...
## Causes and Risk Factors (150 words)
## Symptoms and Warning Signs (200 words - bold red flags)
## Diagnosis at Faraz Hospital (150 words - BodyTom CT, Zeiss microscope)
## Treatment Options (250 words - evidence-based, conservative to surgical)
## Recovery and Outcomes (150 words)
## Frequently Asked Questions (5 Q&As, 200 words)
## When to See a Neurosurgeon - Conclusion + CTA (100 words)
...
Disclaimer: Educational content only. Consult your neurosurgeon for specific advice."""
    ...
    for attempt in range(3):
        try:
            print(f"  Claude attempt {attempt+1}/3 | model=claude-sonnet-4-5 | timeout=180s")
            r = requests.post('https://api.anthropic.com/v1/messages',
                headers={'x-api-key':CLAUDE_KEY,'anthropic-version':'2023-06-01','Content-Type':'application/json'},
                json={
            ...
            if r.status_code == 200:
                t = r.json()['content'][0]['text']
                print(f"Claude SUCCESS: {len(t.split())} words")
                return t
            print(f"Claude error {r.status_code}: {r.text[:300]}")
        except requests.exceptions.ReadTimeout:
            print(f"Timeout attempt {attempt+1}. Waiting 15s...")
    ...
    try:
        r = requests.post('https://api.anthropic.com/v1/messages',
            ...
            json={'model':'claude-sonnet-4-5','max_tokens':1500,
                  'messages':[{'role':'user','content':f'Social media suite for "{topic}". Based on: {content[:500]}...\nCreate: Facebook EN (180w), Facebook Urdu (180w), Instagram + 15 hashtags, Twitter thread (5 tweets), WhatsApp EN+Urdu (80w each). All branded: {AUTHOR_BRAND}, WhatsApp +923458254232.'}]},
            timeout=120)
        if r.status_code == 200:
            s = r.json()['content'][0]['text']
            print(f"Social: {len(s.split())} words")
...
def main():
    print("="*60)
    print("THE NEURO COUNCIL v2.1 - Production Engine")
    print(f"{datetime.now().strftime('%A %d %B %Y %H:%M PKT')}")
    print(f"Type: {CONTENT_TYPE} | Mode: {PUBLISH_MODE}")
    print(f"Model: claude-sonnet-4-5 | Timeout: 180s")
    print("="*60)
    ...
    print("\n"+"="*60)
    print(f"COMPLETE - {wc:,} words | claude-sonnet-4-5 | timeout=180s")
    if result:
        print(f"URL: {result.get('url','')}")
    print("="*60)
...
def grok_research(topic):
    ...
    try:
        r = requests.post('https://api.x.ai/v1/chat/completions',
            headers={'Authorization': f'Bearer {GROK_KEY}', 'Content-Type': 'application/json'},
            json={'model':'grok-3','max_tokens':800,'temperature':0.2,
                  'messages':[{'role':'user','content':f'Research neuroscience article: "{topic}". Latest trials 2023-2026, statistics, AAN/NICE/AANS guidelines, 5 PubMed citations with PMID, Pakistan context. Max 500 words.'}]},
            timeout=60)
        b = r.json()['choices'][0]['message']['content']
        print(f"Grok: {len(b.split())} words")
        return b
    ...
    try:
        r = requests.post('https://api.anthropic.com/v1/messages',
            ...
            json={'model':'claude-3-5-sonnet-20241022','max_tokens':1500,
                  'messages':[{'role':'user','content':f'Create social media posts for "{topic}".\nBased on: {content[:400]}...\n\nGenerate:\nFACEBOOK EN (180 words, end with WhatsApp +923458254232):\nFACEBOOK UR (180 words Urdu Nastaliq):\nINSTAGRAM (120 words + 15 hashtags incl #NeurosurgeryPakistan #DrWasifMalik):\nTWITTER THREAD (5 tweets numbered):\nWHATSAPP (80 words EN + 80 words Urdu):\n\nAll branded as {AUTHOR_BRAND}.'}]},
            timeout=120)
        if r.status_code == 200:
            s = r.json()['content'][0]['text']
            print(f"Social: {len(s.split())} words")
